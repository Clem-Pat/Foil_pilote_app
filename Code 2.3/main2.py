...
"""
revoir les fonctions : experience, excel, initialiser_pot
...
# 'valeur_consigne' évolue dans [-90,90]
...
class tkinterButton(tk.Button):
    ...
    def init(self):
        ...
        global zero, quatre_vingt_dix
        if self.id == 0:
            if self.state == 'off':
                zero = Potentiometre.val
                self.config(bg='grey75', fg='grey')
                self.state = 'on'
            else:
                pass
        elif self.id == 1:
            if L_Buttons_init[0].state == 'on':
                quatre_vingt_dix = Potentiometre.val
                fenetre.destroy()
            else:
                pass
    ...
    def automatique(self):
        if self.state == 'off':
            ...
            L_Entry[0].x, L_Entry[1].x = 147, 335
            for i in range(2):
    ...
    def envoyer_echelon(self):
        global mode_experience, L_temps, L_angle, L_distance, acquerir, app, t0_clear_Canvas
...
class tkinterScale(tk.Scale):
    ...
    def __init__(self, id, application):
        ...
        if self.id == 0:
            ...
            self.config(label='Echelon en degrés', orient='vertical', from_=90, to=-90, cursor='hand2', font='GROBOLD.ttf 10', state='normal',
...
class tkinterMenu(tk.Frame):
    ...
    def enregistrer_sous(self):
        global path, console_text_1, L_Canvas
...
def chronometre(t0):
    ...
    reste = t - tiTuple[3] * 3600.0 - tiTuple[4] * \
        60.0 - tiTuple[5] * 1.0  # Recupération du reste
...
def main():
    global app, path, board, L_Buttons, L_Labels, L_Scales, L_Canvas, L_Entry, L_Labels_init, L_Buttons_init, mode_experience, valeur_consigne, longueur, hauteur, longueur2
    global hauteur2, port_usb, running, Servo, Potentiometre, Capteur, L_temps, L_angle, L_distance, consigne, acquerir
    ...
    acquerir, zero, quatre_vingt_dix = 0, 0, 0

    # consigne et Listes de valeurs à envoyer à excel
    consigne, L_temps, L_angle, L_distance = 0, [], [], []

    running = True
    t0 = time.time()
    # permet d'avoir le bon nombre de valeurs dans le fichier excel (il y en aurait beaucoup si on append à toutes les boucles)
    numero_boucle = 0
    while running == True:
        numero_boucle += 1
        ...
        try:
            ...
            L_Canvas[1].itemconfig(donnees_text_servo_val, text=Servo.val - 90)
            # Valeur rendue par le potentiomètre
            L_Canvas[1].itemconfig(donnees_text_pot_val,
                                   text=Potentiometre.val)
            # Valeur rendue par le capteur de distance
            L_Canvas[1].itemconfig(donnees_text_distance_val, text=Capteur.val)

            if acquerir == 1 and numero_boucle % 3 == 0:
                L_temps.append(time.time() - t0)
                L_angle.append(Potentiometre.val)
            ...
            try:  # Si le texte 'fichier excel créé' est dans le canvas depuis assez longtemps, on le supprime
                duree_Canvas = time.time() - t0_clear_Canvas
                if duree_Canvas > 2 and duree_Canvas < 3:
                    L_Canvas[0].itemconfig(console_text_1, text='')
            except:
                pass
