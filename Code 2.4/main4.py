...
class tkinterButton(tk.Button):
    ...
    def __init__(self, id, application):
        ...
        if application == app:
            if self.id == 0:
                ...
                self.state = 'on'
                self.x, self.y = 650, 230
                self.bg, self.fg, self.cursor = 'blue', 'black', 'hand2'
                self.config(text="Aperçu", width=15, height=2, bg=self.bg, fg=self.fg, font="GROBOLD.ttf 11 bold",
                            relief=tk.RAISED, cursor=self.cursor, command=self.apercu)
    ...
    def init(self):
        ...
        if self.id == 0:
            ...
            if L_Buttons_init[0].state == 'on':
                ...
                fenetre.destroy()
                L_Canvas[0].itemconfig(
                    console_text_1, text='Potentiomètre initialisé')
    ...
    def automatique(self):
        if self.state == 'off':
            ...
            L_Buttons[3].bg, L_Buttons[3].fg, L_Buttons[3].cursor = 'grey', 'grey75', 'arrow'
            L_Buttons[3].config(bg=L_Buttons[3].bg,
                                fg=L_Buttons[3].fg, cursor=L_Buttons[3].cursor)

            L_Buttons[5].bg, L_Buttons[5].fg, L_Buttons[5].cursor = 'light blue', 'black', 'hand2'
            ...
            L_Buttons[7].x, L_Buttons[8].x = 78, 247
            for i in range(6, 9):
            ...
            L_Buttons[3].bg, L_Buttons[3].fg, L_Buttons[3].cursor = 'light blue', 'black', 'hand2'
            L_Buttons[3].config(bg=L_Buttons[3].bg,
                                fg=L_Buttons[3].fg, cursor=L_Buttons[3].cursor)
            ...
            L_Buttons[7].x, L_Buttons[8].x = 1800, 1800
            for i in range(6, 9):
                L_Buttons[i].place(x=L_Buttons[i].x, y=L_Buttons[i].y)
    ...
    def envoyer_echelon(self):
        ...
        if self.bg == 'blue':  # On a cliqué sur le bouton envoyer echelon
            ...
            L_Buttons[4].bg, L_Buttons[4].fg, L_Buttons[4].cursor = 'red', 'black', 'hand2'
            L_Buttons[4].config(bg=L_Buttons[4].bg,
                                fg=L_Buttons[4].fg, cursor=L_Buttons[4].cursor)

            L_Buttons[5].bg, L_Buttons[5].fg, L_Buttons[5].cursor, L_Buttons[5].state = 'light blue', 'black', 'hand2', 'off'
            L_Buttons[5].config(bg=L_Buttons[5].bg,
                                fg=L_Buttons[5].fg, cursor=L_Buttons[5].cursor)

            L_Buttons[6].bg, L_Buttons[6].fg, L_Buttons[6].cursor = 'blue', 'black', 'hand2'
            L_Buttons[6].config(bg=L_Buttons[6].bg, state='normal',
                                fg=L_Buttons[6].fg, cursor=L_Buttons[6].cursor)
...
def main():
    global app, path, board, L_Buttons, L_Labels, L_Scales, L_Canvas, L_Entry, L_Labels_init, L_Buttons_init, mode_experience, valeur_consigne, afficher_apercu
    global port_usb, running, Servo, Potentiometre, Capteur, consigne, L_temps_brut, L_temps_net, L_angle, L_distance, L_servo, acquerir, zero, quatre_vingt_dix
    global L_temps_brut_passe, L_angle_passe, L_distance_passe, L_servo_passe
    ...
    port_usb = 'COM5'
    path = 'C:/Users/cleme/Desktop/Documents/1. DOCUMENTS CLEMENT/TRAVAIL/Post BAC/MP/TIPE/Modèle expérimental/Info/Arduino-Python-Excel CODE 2/Code 2.4'
    ...
    consigne, L_temps_brut, L_temps_net, L_angle, L_distance, L_servo = 0, [], [], [], [], []
    L_temps_brut_passe, L_angle_passe, L_distance_passe, L_servo_passe = [],[],[],[]

    afficher_apercu, running = True, True
    t0 = time.time()
    ...
    while running == True:
        numero_boucle += 1
        # récupérer les valeurs des objets arduino
        get_from_board(board, Servo, Potentiometre, Capteur)

        t = chronometre(t0)
        valeur_servo = Servo.val - 90
        try:
            valeur_potentiometre = ((float(Potentiometre.val) - zero) * (90 - 0)) / ((quatre_vingt_dix - zero))
            # réduire les chiffres significatifs # au début, le type est 'NoneType', d'ou l'exception
            texte_pot = "%.3f" % valeur_potentiometre
        except:
            valeur_potentiometre = Potentiometre.val
            texte_pot = valeur_potentiometre
        try:
            # réduire les chiffres significatifs # au début, le type est 'NoneType', d'ou l'exception
            text_capt = "%.3f" % Capteur.val
        except:
            text_capt = Capteur.val
        ...
        try:
            ...
            L_Canvas[1].itemconfig(donnees_text_distance_val, text=text_capt)
            ...
            if acquerir == 1 and numero_boucle % 3 == 0:  # réduire le nombre de valeurs acquises
                ...
                L_servo.append(valeur_consigne_angle)

            #Les valeurs en temps réel
            L_temps_brut_passe.append(time.time() - t0)
            L_angle_passe.append(valeur_potentiometre)
            L_distance_passe.append(Capteur.val)
            L_servo_passe.append(valeur_consigne_angle)

            # enlever au fur et à mesure les valeurs du début
            if numero_boucle > 100:
                L_temps_brut_passe.pop(0)
                L_angle_passe.pop(0)
                L_distance_passe.pop(0)
                L_servo_passe.pop(0)
